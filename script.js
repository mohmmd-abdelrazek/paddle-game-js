...
let score = 0, boardWidth, isLeftArrowPressed = false, isRightArrowPressed = false, isGameRunning = false, isGamePaused = false, animationFrameId;
...
function move() {
    ...
    isLeftArrowPressed && (document.getElementById("board")).offsetLeft > 0 && (boardX -= boardSpeed);
    isRightArrowPressed && (document.getElementById("board")).offsetLeft < (container.offsetWidth - (document.getElementById("board")).offsetWidth) && (boardX += boardSpeed);
    ...
    if ((document.getElementById("board")).offsetTop - ball.offsetHeight == ball.offsetTop && boardCollision()) {
            ballSpeedY *= -1;
            score++;
...
function setPosition() {
    ...
    boardX= container.offsetWidth / 2 - (document.getElementById("board")).offsetWidth / 2;
    (document.getElementById("board")).style.left = boardX + "px";
    ballX= container.offsetWidth / 2 - ball.offsetWidth / 2;
    ball.style.left = ballX + "px";
    ballY= container.offsetHeight - (document.getElementById("board")).offsetHeight - ball.offsetHeight - 20;
    ball.style.top = ballY + "px";
}
...
function boardCollision() {
    return (document.getElementById("board")).offsetLeft - ball.offsetWidth < ball.offsetLeft &&
    (document.getElementById("board")).offsetLeft + (document.getElementById("board")).offsetWidth > ball.offsetLeft;
}
...
document.addEventListener("keydown", (e) => (e.key === "Escape" || e.key === "Esc") && cancelGame());
